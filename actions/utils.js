 ...
 * Handles cases where GraphQL Mesh wraps arguments in an '__ow_body'.
 *
...
function getParams(params) {
  let jsonBody = params;
  if (params.__ow_body != undefined) {
    try {
      jsonBody = JSON.parse(params.__ow_body)
    } catch (error) {
      return params
    }
  }
...
/**
 * Create organization-scoped keys for data isolation
 * @param {string} key - Base key
 * @param {string} orgId - Organization ID for scoping
 * @returns {string} Org-scoped key
 */
function getOrgScopedKey(key, orgId) {
